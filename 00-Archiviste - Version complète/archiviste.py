...
def rename_file():
    ...
    if not (year and site and nomenclature and emitter and obj and ref):
        messagebox.showwarning("Attention", "Veuillez remplir toutes les cases.")
        return

    new_file_name = f"{year}-{site}-{nomenclature}-{emitter}-{obj}-{ref}{os.path.splitext(selected_file)[1]}"
...
# Champ "Site"
ttk.Label(frame_top, text="Site :").grid(row=1, column=2, padx=5, pady=5, sticky=tk.W)
site_entry = ttk.Entry(frame_top)
site_entry.grid(row=1, column=3, padx=5, pady=5)

# Nomenclature avec bouton pour ouvrir la popup d'information
ttk.Label(frame_top, text="Nomenclature :").grid(row=1, column=4, padx=5, pady=5, sticky=tk.W)
nomenclature_entry = ttk.Entry(frame_top)
nomenclature_entry.grid(row=1, column=5, padx=5, pady=5)
...
nomenclature_help_button.grid(row=1, column=6, padx=5, pady=5)

# Autres champs de texte émetteur
ttk.Label(frame_top, text="Émetteur :").grid(row=1, column=7, padx=5, pady=5, sticky=tk.W)
emitter_entry = ttk.Entry(frame_top)
emitter_entry.grid(row=1, column=8, padx=5, pady=5)
